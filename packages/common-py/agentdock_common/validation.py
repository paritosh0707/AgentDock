"""
AgentDock Validation Utilities

This module provides reusable validation functions used across AgentDock packages.
All validation functions raise ValidationError on invalid input.

Usage:
    from agentdock_common.validation import validate_entrypoint, validate_agent_name
...
def validate_entrypoint(entrypoint: str) -> Tuple[str, str]:
    ...
    if len(parts) != 2:
        raise ValidationError(
            "Entrypoint must have exactly one ':' separator. "
            f"Got: '{entrypoint}'"
        )
...
def validate_agent_name(name: str) -> None:
    ...
    if name.startswith("-") or name.endswith("-"):
        raise ValidationError(
            f"Agent name cannot start or end with a hyphen. Got: '{name}'"
        )
