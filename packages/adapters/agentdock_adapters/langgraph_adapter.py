...
Enables AgentDock to invoke LangGraph agents through a uniform interface.
...
from agentdock_common import validate_entrypoint, get_logger
...
class LangGraphAdapter:
    ...
    def _validate_invoke_signature(self) -> bool:
        ...
        """
        invoke_method = getattr(self._runner, 'invoke')
        ...
        try:
            ...
            if len(params) < 1:
                logger.error(
                    "Invalid invoke() signature: No parameters",
                    signature=str(sig)
                )
                raise InvalidAgentError(
            ...
            if self._supports_config:
                logger.debug(
                    "Agent supports config parameter",
                    signature=str(sig),
                    params=params
                )
            else:
                logger.debug(
                    "Agent does not support config parameter",
                    signature=str(sig),
                    params=params
                )
            ...
            logger.warning(
                "Could not inspect invoke() signature. "
                "Assuming no config support.",
                error=str(e)
            )
    ...
    def load(self, entrypoint: str) -> None:
        ...
        if not callable(getattr(self._runner, 'invoke')):
            agent_type = type(self._runner).__name__
            logger.error("Agent invoke is not callable", agent_type=agent_type)
            raise InvalidAgentError(
                f"Agent .invoke() must be callable. Got type: {agent_type}"
            )
        ...
        self._supports_streaming = hasattr(self._runner, 'stream') and callable(
            getattr(self._runner, 'stream')
        )
        self._supports_async = hasattr(self._runner, 'ainvoke') and callable(
            getattr(self._runner, 'ainvoke')
        )
